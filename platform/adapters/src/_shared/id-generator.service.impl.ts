...
import { DOMAIN_PREFIX_MAP, isValidDomainId } from '@alliora/core';
import { ulid } from 'ulid';
import type { IdGeneratorService } from '@alliora/services/_shared';
...
export class DefaultIdGeneratorService implements IdGeneratorService {
  ...
  capId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.capability);
  }
  dcsId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.decision);
  }
  gatId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.stagegate);
  }
  expId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.experience);
  }
  sitId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.sit);
  }
  altId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.alert);
  }
  bioId(): string {
    return generateIdWithPrefix(DOMAIN_PREFIX_MAP.workforce);
  }
  rptId(): string {
